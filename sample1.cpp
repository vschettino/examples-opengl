...
float Xmin	= -10, Xmax	= 10; //Range of variable x that we sample
float Ymin	= -10, Ymax	= 10; //Range of variable y that we sample
...
float eye_x = 20, eye_y = 16, eye_z = 16; //Modelview (camera extrinsic) parameters
...
void draw() //Render the height plot (GLUT callback function)
{
	...
	float light[4] = {0,1,2,0}; //3. Enable and set one directional light
  ...
  glEnable(GL_COLOR_MATERIAL); //4. Tell OpenGL to use the values passed by glColor() as material properties
	...
	for(float x=Xmin;x<=Xmax-dx;x+=dx) //2. Render the height plot as a set of quads
	 for(float y=Ymin;y<=Ymax-dy;y+=dy)
	 {
		...
		glColor4f(1,0,0,0.9); //Draw the quad filled with green
...
int main(int argc,char* argv[])	//Main program
{
   ...
   glutCreateWindow("1. Graficos Simples"); //4. Create our window
